...
def train(generator, discriminator, g_optim, d_optim, dataset, step, iteration=0, startpoint=0, used_sample=0,
    ...
    fake = SummaryWriter('runs/fake')
    real = SummaryWriter('runs/real')
    ...
    while used_sample < n_sample_total:
        ...
        if iteration % n_show_loss == 0:
            ...
            img_grid = torchvision.utils.make_grid(fake_image[:4], normalize=True)
            fake.add_image('fake images', img_grid)
            fake.add_scalar('fake loss', g_losses)
